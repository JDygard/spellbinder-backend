...
class User {
    constructor(id, username, socket, characters = [], currentCharacter = null, gameState = null, inMatch = false, board = [], combos = []) {
        this.id = id;
...
const setupSocket = (io, authenticateSocket) => {
    io.use(authenticateSocket).on('connection', (socket) => {
        ...
        initializeUser(socket).then((newUser) => {
            // USER DATA EXCHANGE =======================================================
            socket.on("requestPlayerData", () => {
                socket.emit("playerData", { event: "playerData", playerData: newUser.characters });
            });
            ...
            socket.on('createCharacter', async (characterData) => {
                const newCharacter = generateNewCharacter(characterData);
                await createCharacter(socket.user.username, socket.user.id, newCharacter);
                const updatedCharacters = await getCharactersByUsername(socket.user.username);
                socket.emit('characters', updatedCharacters);
            });
            ...
            socket.on('selectCharacter', (characterId) => {
                ...
                const selectedCharacter = user.characters.find((char) => char.characterId === characterId);
                if (selectedCharacter) {
                    updateUser(socket.id, { currentCharacter: selectedCharacter });
